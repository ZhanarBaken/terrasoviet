...
### Режим 1 — Автоматический (без дисплея, для серверов)

Граница карты определяется автоматически по яркостному профилю изображения:
...
### Режим 2 — Интерактивный кроп (`--crop`)

Открывает окно с картой — нужно кликнуть углы карты по периметру (обходя врезки, легенду), затем закрыть окно. Точки сохраняются в `data/polygon_points.txt` и при следующих запусках окно не открывается повторно.
...
> Чтобы перерисовать границу — удали `data/polygon_points.txt` и запусти снова с `--crop`.
...
| `--crop` | Интерактивный кроп границы карты (опционально) |
