...
**При первом запуске** откроется интерактивное окно — нужно кликнуть углы карты по периметру (обходя врезки и легенду), затем закрыть окно. Координаты сохранятся в `data/polygon_points.txt` и при следующих запусках окно не открывается.
