...
# через файл (example/sheets.json уже лежит в репо для тестовой карты)
python main.py --sheets example/sheets.json ...
```
...
Советские карты содержат врезки, легенды и рамки, которые мешают автоматической детекции границ. Полностью надёжную автодетекцию реализовать не успели, поэтому основной режим — ручная разметка: открывается окно с картой, кликаешь углы по периметру (обходя врезки и легенду), закрываешь окно. Точки сохраняются в `example/polygon_points.txt` — при следующих запусках окно не открывается повторно, всё работает без участия пользователя.

```bash
python main.py \
  --map    example/map.jpg \
  --legend example/legenda.jpg \
  --sheets "M-43-В,M-42-Г,L-43-А,L-42-Б" \
  --output results/
```

> Чтобы перерисовать границу — удали `example/polygon_points.txt` и запусти снова.

### Режим 2 — Автоматический (`--no-crop`)

Эвристика на основе яркостного профиля — работает на простых картах без врезок, но ненадёжна на сложных. Не требует дисплея.
...
### Шаг 4 — Кроп границы карты (`pipeline/georeference.py`)
Советские карты содержат врезки и декоративные рамки — автодетекция границ ненадёжна на таких данных. Реализован интерактивный кроп: **первый запуск** открывает окно, пользователь кликает углы карты вручную (обходя врезки). Координаты сохраняются в `example/polygon_points.txt`.  
**Следующие запуски:** читает `polygon_points.txt` напрямую, без окна.
...
│   ├── map.jpg               # карта (растр)
│   ├── legenda.jpg           # легенда
│   ├── sheets.json           # советские номенклатурные листы
...
При смене карты удали `example/polygon_points.txt` — при следующем запуске снова откроется окно для разметки границ.
